...
function sumOfCodes(str) {
  return str
    ? str.split('').reduce((acc, _, index) => acc + str.charCodeAt(index), 0)
    : 0;
}
...
function countVowels(str) {
  const matchArr = str.match(/[aeiouy]/gi);
...
function extractEmails(str) {
  const emailsErr = str.split(';');

  return emailsErr.length ? emailsErr : str;
}
...
function encodeToRot13(str) {
  ...
  return str
    .split('')
    .map((char) => {
      const charIndex = inputRot13.indexOf(char);
      return charIndex === -1 ? char : outputRot13[charIndex];
    })
    .join('');
}
...
function getCardId(value) {
  const cardNumbers = 'A234567891JQK';
  const cardSuits = '♣♦♥♠';

  return (
    cardNumbers.indexOf(value.at(0)) +
    cardSuits.indexOf(value.at(-1)) * cardNumbers.length
  );
}
